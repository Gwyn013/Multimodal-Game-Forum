<div class="{{{ if config.theme.stickyToolbar }}}sticky-tools{{{ end }}} mb-3">
	<nav class="topic-list-header d-flex flex-nowrap my-2 p-0 border-0 rounded">
		<div class="d-flex flex-row p-2 postBar-bg-light gap-1 border rounded w-100">
			...
			<div class="d-flex gap-1 align-items-center">
				{{{ if template.category }}}
					...
					<a href="{config.relative_path}/compose?cid={cid}" component="category/post" id="new_topic" class="btn reply-btn btn-sm text-nowrap" data-ajaxify="false" role="button">[[category:new-topic-button]]</a>
					{{{ end }}}
				{{{ else }}}
					{{{ if canPost }}}
				...
				<a component="category/post/guest" href="{config.relative_path}/login" class="btn btn-sm reply-btn">[[category:guest-login-post]]</a>
				{{{ end }}}
			</div>
		</div>
	</nav>
