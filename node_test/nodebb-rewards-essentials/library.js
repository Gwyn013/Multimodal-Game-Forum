...
plugin.onTopic = function (data) {
	const { uid } = data.topic;

	rewards.checkConditionAndRewardUser({
		uid: uid,
		condition: 'essentials/user.topiccount',
		method: function (callback) {
			user.getUserField(uid, 'topiccount', callback);
		},
	});
};

/*
plugin.onWatching = function (data) {
	const { uid } = data.topic;
