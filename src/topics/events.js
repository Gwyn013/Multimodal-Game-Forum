...
function renderUser(event) {
	...
	return `${helpers.buildAvatar(event.user, '16px', true)} <a href="${relative_path}/user/${event.user.userslug}">${event.user.displayname?event.user.displayname:event.user.username}</a>`;
}
